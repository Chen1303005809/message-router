...
2. 管理员在事件中心录入：
   ...
   - 人员与内部团队的映射；
   - 团队管理员。
3. 每个研发群由团队管理员执行一次 `@机器人 绑定研发团队`。机器人从回调取得 `chatid` 并绑定到内部 `dev_team_id`。
4. 每位需要接收主动消息的咨询人员先与机器人发生一次单聊交互。
5. 启用前的检查页列出尚未初始化的用户或研发群；缺少目标会话时禁止创建事件。
...
6. 正式回复先写入时间线和投递束记录。投递束中的全部 Markdown 与图片项均推送成功后，才把 `waiting_on` 改为 `consult`。
...
| `wecom_channels` | `team_id`, `chatid`, `active`, `initialized_at` | 研发团队当前群聊通道 |
...
- 研发群目标使用团队当前 `chatid`；咨询目标使用当前咨询经办人的 `userid`。
...
4. `/admin`（兼容入口 `/settings/routing`）：总管理员使用，配置企业微信成员、咨询队列、研发责任团队、成员/团队管理员授权和研发群绑定状态。首次部署可通过 `/setup` 初始化总管理员。
...
1. 在同一数据库事务中追加 `case_entries`、有序内容片段，并创建 `deliveries(pending)` 及对应投递项。
2. 提交事务后由 `bot-worker` 拉取待投递记录。
3. 按 `delivery_items.position` 依次发送 Markdown 和图片；已成功的投递项不重复发送。
4. 所有投递项成功后才把整束标记为 `sent`，并按消息语义更新 `waiting_on`。
